...
import { haversineKm, geoScoreFromKm, timeScoreFromDays, attributeScore, MATCH_WEIGHTS, CONFIDENCE_THRESHOLDS, } from '@perros/shared';
...
const ALL_SIZE_KWS = Object.values(SIZE_KEYWORDS).flat();
...
const fmt = (v) => v === null ? 'null' : v ? 'true' : 'false';
// ─── Matching básico (sin IA) ─────────────────────────────────────────────────
async function basicProcessSighting(sightingId) {
    ...
    const nearbyCases = activeCases.filter(c => {
        const dist = haversineKm(sighting.locationLat, sighting.locationLng, c.lastSeenLat, c.lastSeenLng);
        return dist <= c.searchRadiusKm;
    });
    const results = [];
    for (const lostCase of nearbyCases) {
        const distKm = haversineKm(sighting.locationLat, sighting.locationLng, lostCase.lastSeenLat, lostCase.lastSeenLng);
        const geo = geoScoreFromKm(distKm, lostCase.searchRadiusKm);
        const daysDiff = Math.abs((sighting.seenAt.getTime() - lostCase.lastSeenAt.getTime()) / 86_400_000);
        ...
        const breedMatch = (hasDesc && lostCase.dog.breed &&
        ...
        const colorMatch = (descHasColor && lostCase.dog.color.length > 0)
        ...
        const sizeMatch = (descHasSize && sizeKws.length > 0)
        ...
        const confidence = total >= CONFIDENCE_THRESHOLDS.high ? 'high' :
        ...
        console.log(`[match:eval] sighting=${sightingId} case=${lostCase.id} dog="${lostCase.dog.name}"` +
...
async function aiProcessSighting(sightingId) {
    ...
    const results = [];
    for (const lostCase of nearbyCases) {
        if (lostCase.dog.photos.length === 0)
            continue;
        ...
        const total = visual * MATCH_WEIGHTS.visual +
...
export async function processSighting(sightingId) {
