...
// ─── Extracción de atributos del perro via GPT-4o Vision ─────────────────────
...
export async function extractDogAttributesFromImage(
  imageUrl: string,
): Promise<DogAttributesFromImage> {
...
Si no hay perro en la imagen, pon hasDogDetected: false y confidence: 0.`,
  ...
  try {
    return JSON.parse(cleaned) as DogAttributesFromImage;
  } catch {
    return {
...
export async function generateImageEmbedding(imageUrl: string): Promise<number[]> {
  ...
  const output = await getReplicate().run(
    'openai/clip-vit-large-patch14:b8afe62952083f9dc7c6d30bfe9b1e48cb3c4be9',
