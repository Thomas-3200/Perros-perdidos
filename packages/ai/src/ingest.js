...
export async function parseImportedCase(importedCaseId) {
    ...
    try {
        ...
        if (imported.sourceType === 'screenshot' || imported.sourceType === 'photo') {
            ...
            if (!imageRes.ok)
                throw new Error(`No se pudo descargar la imagen: ${imageRes.status} ${imageRes.statusText}`);
...
function mapExtracted(raw) {
    ...
    return {
        ...
        seenAt: raw.seenAt ? new Date(raw.seenAt) : undefined,
        photos: [],
        contactInfo: raw.contactInfo,
        reward: raw.reward,
        dogAttributes: raw.dogAttributes,
