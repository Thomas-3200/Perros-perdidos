 ...
 * 0 km → 1.0 | maxKm → 0.0
 */
export function geoScoreFromKm(distanceKm: number, maxKm = 50): number {
 ...
 */
export function timeScoreFromDays(daysDiff: number, maxDays = 30): number {
...
export function attributeScore(params: {
  ...
  if (colorMatch === false) {
    const partials = [breedMatch, sizeMatch].filter((v) => v !== null) as boolean[];
  ...
  const checks = [breedMatch, sizeMatch, colorMatch].filter(
    (v) => v !== null,
  ) as boolean[];
  if (checks.length === 0) return 0.5; // Sin datos suficientes → neutro
